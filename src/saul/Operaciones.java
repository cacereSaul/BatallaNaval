...
public class Operaciones {
	...
	public static void mostrar2(Naves [] barcos) {
		for (int i = 0; i < barcos.length; i++) {
			 System.out.println("Coordenadas del Barco Nº"+(i+1)+":  "+barcos[i].getCoordenada1()+"  "+barcos[i].getCoordenada2());
	...
	public static boolean lugarLibre(String coor, Naves [] barcos){
		...
		for(int i=0; i<barcos.length;i++){
			...
			if(barcos[i].getCoordenada1().equalsIgnoreCase(coor) || barcos[i].getCoordenada2().equalsIgnoreCase(coor)){
				return false;
			}else{
				return true;
			}
	...
	public static int ataques(String coordenada, Naves[] barcos){
		for(int i=0; i<barcos.length; i++){
			if(barcos[i].getCoordenada1().equalsIgnoreCase(coordenada)  ||  barcos[i].getCoordenada2().equalsIgnoreCase(coordenada)){
				return i;
